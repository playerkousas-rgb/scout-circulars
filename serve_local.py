...
歷史：2026-09-16 之前呢度仲掛載 /api/render（PDF → 圖片）；該功能已移除
（見 README「Vercel 用量」），本機亦不再需要 pymupdf。
"""
...
def build_handler(push_config_handler, push_subscriptions_handler):
    ...
    class LocalHandler(SimpleHTTPRequestHandler):
        ...
        def end_headers(self):
            ...
            if not (self._is_push_config() or self._is_push_subscriptions()):
...
def main():
    ...
    server = ThreadingHTTPServer(
        (args.host, args.port),
        build_handler(push_config.handler, push_subscriptions.handler),
    )
