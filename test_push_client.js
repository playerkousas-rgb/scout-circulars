...
(async () => {
  ...
  // iOS needs an installed home-screen app; Android/desktop do not.
  win.navigator.userAgent = 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)';
  assert.strictEqual(client.isSupported(), false);
  assert.strictEqual((await client.status()).needsHomeScreen, true);
  const writesBefore = requestBodies.length;
  await assert.rejects(client.enable({ branches: ['小童軍'], topics: ['activity:other'] }), error => error.code === 'home_screen_required');
  assert.strictEqual(requestBodies.length, writesBefore, 'no remote write before installation');
  win.navigator.standalone = true;
  assert.strictEqual(client.isSupported(), true);
  assert.strictEqual((await client.status()).standalone, true);
  win.navigator.standalone = false;
  win.navigator.userAgent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X)';
  win.navigator.platform = 'MacIntel';
  win.navigator.maxTouchPoints = 5;
  assert.strictEqual(client.isSupported(), false, 'iPad desktop user agent also needs installation');
  win.matchMedia = () => ({ matches: true });
  assert.strictEqual(client.isSupported(), true, 'standalone display mode enables installed iPad');
  win.matchMedia = () => ({ matches: false });
  win.navigator.platform = 'Linux';
  win.navigator.userAgent = 'Mozilla/5.0 (Linux; Android 14)';
  assert.strictEqual(client.isSupported(), true, 'Android supports push without mandatory installation');
